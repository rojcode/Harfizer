...
exports.RussianLanguagePlugin = void 0;
class RussianLanguagePlugin {
    ...
    convertYear(year) {
        ...
        if (thousands === 1) {
            ...
            const form = [2, 3, 4].includes(thousands % 10) &&
                ![12, 13, 14].includes(thousands % 100)
                ? "тысячи"
                : "тысяч";
            thousandsPart = this.convertNumber(thousands) + " " + form;
        }
        let remainderPart = remainder > 0 ? " " + this.convertNumber(remainder) : "";
        return thousandsPart + remainderPart;
    }
    convertNumber(input, options) {
        const effectiveOptions = { ...options };
        const zeroWord = effectiveOptions.customZeroWord || RussianLanguagePlugin.ZERO_WORD;
        ...
        const triples = RussianLanguagePlugin.splitIntoTriples(integerPart);
        const wordParts = [];
    ...
    getHourSuffix(hour) {
        ...
        else if ([2, 3, 4].includes(hour % 10) &&
    ...
    getMinuteSuffix(minute) {
        ...
        else if ([2, 3, 4].includes(minute % 10) &&
...
exports.RussianLanguagePlugin = RussianLanguagePlugin;
RussianLanguagePlugin.DEFAULT_SEPARATOR = " ";
RussianLanguagePlugin.ZERO_WORD = "ноль";
RussianLanguagePlugin.NEGATIVE_WORD = "минус";
RussianLanguagePlugin.SCALE = [
    "",
    "тысяча",
    "миллион",
    "миллиард",
...
RussianLanguagePlugin.HUNDREDS = [
    "",
    "сто",
    "двести",
    "триста",
