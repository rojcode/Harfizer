...
exports.PersianLanguagePlugin = void 0;
/**
...
class PersianLanguagePlugin {
    ...
    convertFractionalPart(fraction, decimalSuffixes) {
        ...
        const localDecimalSuffixes = decimalSuffixes ?? PersianLanguagePlugin.DEFAULT_DECIMAL_SUFFIXES;
    ...
    convertNumber(input, options) {
        ...
        const lexicon = effectiveOptions.customLexicon ?? PersianLanguagePlugin.DEFAULT_LEXICON;
        const decimalSuffixes = effectiveOptions.customDecimalSuffixes ??
            PersianLanguagePlugin.DEFAULT_DECIMAL_SUFFIXES;
        const zeroWord = effectiveOptions.customZeroWord ?? PersianLanguagePlugin.ZERO_WORD;
        ...
        const triples = PersianLanguagePlugin.splitIntoTriples(inputStr);
        const wordParts = [];
        // Convert each triple to words, then append the corresponding scale (e.g. هزار, میلیون).
        for (let i = 0; i < triples.length; i++) {
            const converted = this.convertTripleToWords(triples[i], lexicon);
            if (converted !== "") {
 ...
 */
PersianLanguagePlugin.DEFAULT_LEXICON = [
