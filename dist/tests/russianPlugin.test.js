...
const russianPlugin_1 = require("../plugins/russianPlugin");
describe("CoreConverter with RussianLanguagePlugin", () => {
     ...
     */
    beforeEach(() => {
        const russianPlugin = new russianPlugin_1.RussianLanguagePlugin();
        converter = new core_1.CoreConverter(russianPlugin);
    });
    test("Convert positive number with decimal part (12345.67)", () => {
        ...
        expect(result).toBe("двенадцать тысяча триста сорок пять запятая шесть семь");
    });
    test("Convert zero", () => {
    ...
    test('Process input with comma separators ("1,234,567.89")', () => {
        ...
        expect(result).toBe("один миллион двести тридцать четыре тысяча пятьсот шестьдесят семь запятая восемь девять");
    });
    ...
    test('Process input with dash separators ("1-2343-53123")', () => {
        ...
        expect(result).toBe("один миллиард двести тридцать четыре миллион триста пятьдесят три тысяча сто двадцать три");
    });
